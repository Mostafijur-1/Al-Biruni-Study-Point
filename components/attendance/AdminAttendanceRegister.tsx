...
export function AdminAttendanceRegister() {
  ...
  const load = useCallback(async () => {
    ...
    else { setMessage(getApiErrorMessage(result.payload, "Attendance register could not be loaded.")); setError(true); }
    setLoading(false);
  }, [batchId, date]);
  ...
  async function assignExistingIds() {
    if (!window.confirm("Assign permanent IDs to all existing active batch students who do not have one?")) return;
    ...
    if (result.ok && isApiSuccess(result.payload)) {
      await load();
      setMessage(`${result.payload.data.assignedCount} permanent student ID(s) assigned.`);
      setError(false);
    } else {
      setMessage(getApiErrorMessage(result.payload, "Existing student IDs could not be assigned."));
      setError(true);
    }
    ...
    <section className="rounded-2xl border border-border bg-card shadow-[var(--shadow-sm)]" aria-labelledby="admin-attendance-title">
      <header className="border-b border-border p-4 sm:p-5">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div><p className="flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-accent-foreground"><Sheet className="size-4" /> Attendance register</p><h2 id="admin-attendance-title" className="mt-1 text-xl font-black text-primary">All batches and classes</h2><p className="mt-1 text-xs text-muted">Submitted attendance appears here as a permanent spreadsheet-style register.</p></div>
        ...
        <div className="mt-4 grid gap-2 md:grid-cols-[1fr_220px_180px]">
          <label className="relative"><span className="sr-only">Search attendance</span><Search className="absolute left-3 top-3.5 size-4 text-muted" /><Input value={search} onChange={(event) => setSearch(event.target.value)} className="pl-9" placeholder="Search ID, student, batch or subject" /></label>
          <select aria-label="Filter by batch" value={batchId} onChange={(event) => setBatchId(event.target.value)} className="h-11 rounded-xl border border-input bg-white px-3 text-sm"><option value="">All batches</option>{data.batches.map((batch) => <option key={batch.id} value={batch.id}>{batch.name}</option>)}</select>
          <label className="relative"><span className="sr-only">Filter by date</span><CalendarDays className="absolute left-3 top-3.5 size-4 text-muted" /><Input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="pl-9" /></label>
        </div>
        {data.missingStudentCodeCount > 0 && <Alert className="mt-3 border-amber-300 bg-amber-50 text-amber-900"><span>{data.missingStudentCodeCount} existing student(s) do not have a permanent ID yet. New batch enrollments receive one automatically.</span><Button type="button" size="sm" variant="outline" className="ml-3" disabled={assigningIds} onClick={() => void assignExistingIds()}><RefreshCw className={cn("size-4", assigningIds && "animate-spin")} /> Assign existing IDs</Button></Alert>}
        {data.truncated && <Alert className="mt-3 border-amber-300 bg-amber-50 text-amber-900">Only the latest 5,000 attendance rows are displayed. Use batch or date filters to narrow the register.</Alert>}
        {message && <Alert variant={error ? "destructive" : "success"} className="mt-3">{message}</Alert>}
      </header>
      <div className="max-h-[620px] overflow-auto">
        <table className="w-full min-w-[900px] border-collapse text-left text-sm">
          <thead className="sticky top-0 z-10 bg-secondary text-xs uppercase tracking-wide text-muted"><tr><th className="w-12 border-b border-r border-border px-3 py-2 text-center">#</th><th className="border-b border-r border-border px-3 py-2">Student ID</th><th className="border-b border-r border-border px-3 py-2">Student name</th><th className="border-b border-r border-border px-3 py-2">Date</th><th className="border-b border-r border-border px-3 py-2">Attendance status</th><th className="border-b border-r border-border px-3 py-2">Batch</th><th className="border-b border-border px-3 py-2">Class / Subject</th></tr></thead>
          <tbody>{loading ? Array.from({ length: 6 }, (_, index) => <tr key={index}><td colSpan={7} className="border-b border-border p-3"><div className="h-5 animate-pulse rounded bg-secondary" /></td></tr>) : visibleRows.length === 0 ? <tr><td colSpan={7} className="p-10 text-center text-muted">No submitted attendance matches these filters.</td></tr> : visibleRows.map((row, index) => (
