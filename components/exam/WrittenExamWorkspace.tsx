    ...
    <div className="grid items-start gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
      <div className="min-w-0 space-y-5">
        <form onSubmit={createExam} className="space-y-4 rounded-3xl border border-border bg-card p-5 shadow-sm">
          ...
          <Button type="submit" className="w-full" disabled={saving || !batchId || !subjectId}><FilePenLine className="size-4" />{saving ? "তৈরি হচ্ছে…" : "Written Exam তৈরি করুন"}</Button>
        </form>
        <section className="min-w-0 space-y-2"><h2 className="font-black text-primary">সব Written Exam</h2>{exams.length === 0 ? <p className="rounded-2xl border border-dashed border-border p-5 text-sm text-muted">এখনো কোনো Written Exam নেই।</p> : exams.map((exam) => <button type="button" key={exam.id} onClick={() => void openExam(exam)} className={`w-full min-w-0 rounded-2xl border bg-card p-4 text-left shadow-sm ${selectedExam?.id === exam.id ? "border-primary ring-2 ring-primary/10" : "border-border"}`}><div className="flex min-w-0 items-start justify-between gap-2"><div className="min-w-0"><b className="block break-words text-primary">{exam.title}</b><p className="mt-1 break-words text-xs text-muted">{exam.batchName} · {exam.subjectName} · {new Date(exam.examDate).toLocaleDateString("en-GB")}</p>{exam.hasQuestionFile && <p className="mt-1 text-xs font-bold text-primary">প্রশ্ন যুক্ত আছে</p>}</div><span className={`shrink-0 rounded-full px-2 py-1 text-[10px] font-black ${exam.isPublished ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-800"}`}>{exam.isPublished ? "Published" : "Draft"}</span></div></button>)}</section>
      </div>
      <section className="min-w-0 overflow-hidden rounded-3xl border border-border bg-card p-5 shadow-sm">
        {!selectedExam ? <div className="grid min-h-96 place-items-center text-center"><div><FilePenLine className="mx-auto size-10 text-muted" /><h2 className="mt-3 text-xl font-black text-primary">একটি Exam Select করুন</h2><p className="mt-1 text-sm text-muted">Marks দেওয়ার জন্য ওই Batch-এর Student list খুলবে।</p></div></div> : <div className="min-w-0 space-y-4"><div className="flex min-w-0 flex-wrap items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-2xl font-black text-primary">{selectedExam.title}</h2><p className="text-sm text-muted">সর্বোচ্চ {selectedExam.totalMarks} Marks · {students.length} জন Active শিক্ষার্থী</p>{selectedExam.hasQuestionFile && <a className="mt-1 inline-block text-xs font-bold text-primary underline" target="_blank" rel="noreferrer" href={`/api/written-exams?examId=${selectedExam.id}&question=true`}>প্রশ্নের Source খুলুন</a>}</div>{selectedExam.isPublished && <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-emerald-100 px-3 py-1.5 text-xs font-black text-emerald-700"><CheckCircle2 className="size-4" />Published</span>}</div>
          ...
          <div className="max-w-full overflow-x-auto overscroll-x-contain rounded-2xl border border-border"><table className="w-full min-w-[680px] table-fixed text-sm"><colgroup><col className="w-32" /><col className="w-48" /><col className="w-36" /><col /></colgroup><thead className="bg-secondary text-left text-xs uppercase tracking-wide text-muted"><tr><th className="px-3 py-3">Student ID</th><th className="px-3 py-3">শিক্ষার্থী</th><th className="px-3 py-3">Marks / {selectedExam.totalMarks}</th><th className="px-3 py-3">Comment</th></tr></thead><tbody>{students.map((student) => <tr key={student.id} className="border-t border-border"><td className="truncate px-3 py-2 font-mono font-black text-primary" title={student.studentCode}>{student.studentCode ?? "—"}</td><td className="break-words px-3 py-2 font-bold">{student.name}</td><td className="px-3 py-2"><Input aria-label={`${student.name} marks`} disabled={selectedExam.isPublished} className="w-full" type="number" min={0} max={selectedExam.totalMarks} value={student.marks ?? ""} onChange={(event) => setStudents((current) => current.map((row) => row.id === student.id ? { ...row, marks: event.target.value === "" ? undefined : Number(event.target.value) } : row))} /></td><td className="min-w-0 px-3 py-2"><Input aria-label={`${student.name} comment`} disabled={selectedExam.isPublished} className="min-w-0" value={student.comment ?? ""} onChange={(event) => setStudents((current) => current.map((row) => row.id === student.id ? { ...row, comment: event.target.value } : row))} /></td></tr>)}</tbody></table></div>
