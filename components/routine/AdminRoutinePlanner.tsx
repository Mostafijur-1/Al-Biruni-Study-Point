...
export function AdminRoutinePlanner() {
  ...
  async function saveRoutine(event: FormEvent) {
    ...
    if (failed) { setError(true); setMessage(getApiErrorMessage(failed.payload, "Routine Save করা যায়নি।")); }
    else {
  ...
  async function endRoutine(id: string) {
    if (!window.confirm("এই Routine Delete করবেন? ভবিষ্যৎ Dashboard থেকে এটি সরে যাবে।")) return;
    setSaving(true);
    const result = await apiFetch("/api/routines", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "end", routineSlotId: id, reason: "Admin Routine Delete করেছেন" }) });
    if (!result.ok || !isApiSuccess(result.payload)) { setError(true); setMessage(getApiErrorMessage(result.payload, "Routine Delete করা যায়নি।")); }
    else { setError(false); setMessage("Routine Delete হয়েছে।"); await load(); }
    setSaving(false);
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[420px_minmax(0,1fr)]">
      <form onSubmit={saveRoutine} className="h-fit space-y-5 rounded-3xl border border-border bg-card p-5 shadow-[var(--shadow-md)] xl:sticky xl:top-24">
        <div className="flex items-center gap-3"><span className="grid size-11 place-items-center rounded-2xl bg-primary text-white"><CalendarPlus className="size-5" /></span><div><h2 className="font-black text-primary">{editingId ? "Routine Edit" : "নতুন Routine"}</h2><p className="text-xs text-muted">Batch → Teacher → Subject</p></div></div>
        ...
        <div className="grid grid-cols-2 gap-3"><div className="space-y-2"><Label htmlFor="start">শুরু</Label><Input id="start" type="time" lang="en" required value={start} onChange={(event) => setStart(event.target.value)} /></div><div className="space-y-2"><Label htmlFor="end">শেষ</Label><Input id="end" type="time" lang="en" required value={end} onChange={(event) => setEnd(event.target.value)} /></div></div>
        ...
        <Button type="submit" className="w-full" size="lg" disabled={saving}>{saving ? "Save হচ্ছে…" : editingId ? "পরিবর্তন Save করুন" : "Routine প্রকাশ করুন"}</Button>
      </form>
      <section className="space-y-4">{loading ? <div className="h-64 animate-pulse rounded-3xl bg-secondary" /> : routines.length === 0 ? <div className="rounded-3xl border border-dashed border-border p-10 text-center text-muted">এখনো কোনো Routine নেই।</div> : routineDays.map((day, index) => { const items = routines.filter((item) => item.weekday === index); if (!items.length) return null; return <div key={day} className="rounded-3xl border border-border bg-card p-5"><h3 className="mb-3 font-black text-primary">{day}</h3><div className="space-y-3">{items.map((item) => <article key={item.id} className="grid gap-3 rounded-2xl border border-border bg-surface p-4 sm:grid-cols-[110px_1fr_auto] sm:items-center"><div><p className="text-lg font-black text-brand-red">{routineTime(item.startMinute)}</p><p className="text-xs text-muted">থেকে {routineTime(item.endMinute)}</p></div><div><p className="font-black text-primary">{item.subject?.nameBn || item.subject?.name}</p><p className="mt-1 text-sm text-muted">{item.batch?.name} • {item.teacher.name}</p><div className="mt-2 flex flex-wrap gap-3 text-xs text-muted"><span className="inline-flex items-center gap-1"><Users className="size-3.5" />{item.eligibleStudentCount ?? 0} জন eligible</span>{item.room && <span className="inline-flex items-center gap-1"><MapPin className="size-3.5" />{item.room}</span>}<span className="inline-flex items-center gap-1"><Clock3 className="size-3.5" />সাপ্তাহিক</span></div></div><div className="flex gap-2"><Button type="button" variant="outline" size="sm" onClick={() => editRoutine(item)}><Pencil className="size-4" /> Edit</Button><Button type="button" variant="outline" size="sm" onClick={() => void endRoutine(item.id)}><Trash2 className="size-4" /> Delete</Button></div></article>)}</div></div>; })}</section>
    </div>
  </div>;
}
