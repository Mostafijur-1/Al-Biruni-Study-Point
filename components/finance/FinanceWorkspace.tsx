...
export function FinanceWorkspace() {
  ...
  async function saveProfile(event: FormEvent) {
    ...
    await save(data?.ledgerMode === "authoritative" && selected.user.role === "student"
      ...
      : { action: "set-profile", userId: selected.user.id, defaultAmountTk: defaultAmount }, "ডিফল্ট মাসিক সেটিং সংরক্ষণ হয়েছে। নতুন মাসে এই পরিমাণ স্বয়ংক্রিয়ভাবে আসবে।");
  }
  ...
  return <div className="space-y-6">
    ...
    {data?.ledgerMode === "shadow" && <Alert>Immutable cash ledger is in shadow mode. The monthly tracker remains authoritative until migration reconciliation is approved.</Alert>}
    {data?.ledgerMode === "authoritative" && <Alert variant="success">Cash ledger is authoritative. Every cash entry creates an immutable transaction and receipt.</Alert>}
