...
type FinanceRecord = {
  user: { id: string; name: string; reference?: string; phone?: string; email?: string; role: "student" | "teacher"; studentClass?: string; isActive: boolean; batch?: { id: string; name?: string } };
...
export function FinanceWorkspace() {
  ...
  function choose(record: FinanceRecord) {
    setSelected(record); setDefaultAmount(record.profile.defaultAmountTk); setMonthAmount(record.payment.amountTk); setNote(record.payment.note || ""); setMessage("");
  }
  ...
  async function saveProfile(event: FormEvent) {
    ...
    await save({ action: "set-profile", userId: selected.user.id, defaultAmountTk: defaultAmount }, "ডিফল্ট মাসিক সেটিং সংরক্ষণ হয়েছে। নতুন মাসে এই পরিমাণ স্বয়ংক্রিয়ভাবে আসবে।");
  }
