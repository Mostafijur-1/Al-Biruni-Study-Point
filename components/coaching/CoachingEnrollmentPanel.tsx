...
export function CoachingEnrollmentPanel() {
  ...
  function chooseStudent(student: Student) {
    ...
    setSelectedEnrollment(undefined); setSelectedStudent(student); setBatchId(batchFromLink); setBatchFilter(batchFromLink); setSubjects([]); setSubjectIds([]); setFeeTk(batches.find((batch) => batch.id === batchFromLink)?.defaultFeeTk ?? 0); setMessage("");
  }
