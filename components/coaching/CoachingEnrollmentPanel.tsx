...
export function CoachingEnrollmentPanel() {
  ...
  async function withdraw() {
    ...
    setSaving(true);
    const result = await apiFetch("/api/enrollments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "withdraw", enrollmentId: selectedEnrollment.id, reason: "Admin Coaching Enrollment Close করেছেন" }) });
    if (!result.ok || !isApiSuccess(result.payload)) { setError(true); setMessage(getApiErrorMessage(result.payload, "Enrollment Close করা যায়নি।")); }
    else { setError(false); setMessage("Coaching Enrollment Close হয়েছে; Platform Account অপরিবর্তিত আছে।"); closeEditor(); await load(); }
    setSaving(false);
  }
