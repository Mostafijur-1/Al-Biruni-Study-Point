...
export function CoachingEnrollmentPanel() {
  ...
  const visibleEnrollments = useMemo(() => enrollments.filter((item) => {
    const text = `${item.student.name} ${item.student.reference || ""}`.toLowerCase();
  ...
  async function save(event: FormEvent) {
    ...
    const body = selectedEnrollment
      ? batchId !== selectedEnrollment.batchId
        ? { action: "transfer", enrollmentId: selectedEnrollment.id, targetBatchId: batchId, subjectIds, reason: "অ্যাডমিন কর্তৃক coaching batch transfer" }
        : { action: "update-subjects", enrollmentId: selectedEnrollment.id, subjectIds, reason: "অ্যাডমিন কর্তৃক কোচিং বিষয় হালনাগাদ" }
  ...
  return <section className="space-y-5" aria-labelledby="coaching-enrollment-title">
    <header className="rounded-3xl bg-[linear-gradient(125deg,#0b2545,#123a6b_70%,#174d82)] p-6 text-white shadow-lg sm:p-8"><p className="text-xs font-black uppercase tracking-[.22em] text-brand-yellow">Coaching enrollment</p><h1 id="coaching-enrollment-title" className="mt-2 text-3xl font-black">কোচিং শিক্ষার্থী ব্যবস্থাপনা</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-white/75">Website registration এবং coaching enrollment আলাদা। Admin ব্যাচ দিলে তবেই শিক্ষার্থী coaching routine, attendance ও fee-এর আওতায় আসবে।</p></header>
