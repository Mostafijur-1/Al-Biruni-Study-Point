...
test("teacher assignment migration is bounded, private, and confirmation guarded", async () => {
  const [source, backfill] = await Promise.all([
    readFile("scripts/migrate-teacher-assignments.ts", "utf8"),
    readFile("lib/db/teacher-assignment-backfill.ts", "utf8"),
  ]);
  ...
  assert.match(backfill, /domain\.isAll\s*\? batches/);
});
