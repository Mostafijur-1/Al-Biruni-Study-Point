...
interface ScopedRecord { organizationId: Types.ObjectId; createdAt: Date }
...
FeePlanSchema.index({ organizationId: 1, code: 1 }, { unique: true });
...
StudentFeeAssignmentSchema.index({ organizationId: 1, studentId: 1, effectiveFrom: 1 }, { unique: true });
...
FinanceInvoiceSchema.index({ organizationId: 1, invoiceNumber: 1 }, { unique: true });
FinanceInvoiceSchema.index({ organizationId: 1, counterpartyId: 1, period: 1, kind: 1 }, { unique: true });
...
CashTransactionSchema.index({ organizationId: 1, idempotencyKey: 1 }, { unique: true });
CashTransactionSchema.index({ organizationId: 1, occurredAt: -1 });
...
LedgerAdjustmentSchema.index({ organizationId: 1, idempotencyKey: 1 }, { unique: true });
...
LedgerExpenseSchema.index({ organizationId: 1, period: 1, category: 1 });
...
CashReceiptSchema.index({ organizationId: 1, receiptNumber: 1 }, { unique: true });
